...
- **Total Costs:** 9900 DOT ~ $75,000  USD

### Milestone 1: Light Client + Relayer

- **Estimated duration:** 4 weeks
- **FTE:**  3
- **Costs:** 3300 DOT (6.875 DOT/hr)
...
| **0d.** | Platform | Provide a detailed description of the game's platform compatibility and the infrastructure setup required to host and run the game, including supported operating systems, hardware requirements, and necessary backend services. |
| 0e. | Article | We will publish an **article**/workshop that explains [...] (what was done/achieved as part of the games bounty). (Content, language, and medium should reflect your target audience described above.) |
| 1. | Library: Etf.js - SnarkJS support | We develop a simple circuit library and add pre-compiles to IPFS. Then we modify the etf.js library to use the [snarkJS](https://github.com/iden3/snarkjs) library to prepare witness files with new inputs on pre-compiled circuits.   |
| 2. | Substrate Runtime: pallet-witness-encryption (part 1) | We develop a new pallet for enabling practical on-chain witness encryption. The pallet functions as described above, where users encode public parameters for circuits and resharings of the associated secret in a pallet, which is then associated with a new on-chain asset class. We adapt this from the work done by [bright](https://brightinventions.pl/blog/zk-snarks-in-substrate-part-1/), using [this zkSnark pallet](https://github.com/bright/zk-snarks-with-substrate/tree/main/pallets/zk-snarks) to get started. Note: This intermediate step does not modify validator logic yet. We leverage the acss scheme already implemented in the Ideal network [here](https://github.com/ideal-lab5/etf-sdk/blob/w3fbls-migration/etf-crypto-primitives/src/dpss/acss.rs). |
...
| 4. |  Demo/Implementation: Basic Secret Sharing Demo | We develop a basic demo to showcase this scheme, wherein we implement a simple secret sharing application. This will allow a 'data owner' to encrypt a message for which decryption keys are provided to a caller if and only if they provide a valid proof. While not exactly a game, it will be a pattern that we will heavily use when developing the game in the next milestone. |
